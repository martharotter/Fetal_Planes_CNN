...
def main():
    ...
    if physical_devices:
        log("Using GPU:", physical_devices)
    else:
        log("Using CPU")
    ...
    if args.model == "cnn":
        ...
        log("Starting run of all three models, this may take a while to run, please be patient")
        run_cnn(metadata_df)
        run_resnet50(metadata_df)
        run_efficientNet(metadata_df)
        log("Finished run of all three models, thanks for your patience!")
    return
